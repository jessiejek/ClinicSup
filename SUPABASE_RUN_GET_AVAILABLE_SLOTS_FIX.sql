...
--
-- Fixes "column reference \"slot_start_time\" is ambiguous"
-- error (code 42702) in get_available_slots RPC.
--
-- Root cause: existing_bookings CTE selected unqualified
-- column names slot_start_time and slot_end_time, which
-- conflicted with the function's RETURNS TABLE output
-- columns of the same name.
--
-- Fix: Alias booking columns as booking_slot_start_time /
-- booking_slot_end_time in the CTE, qualify bookings columns
-- with table alias b., and update the LEFT JOIN to use the
-- aliased names.
--
-- Also includes locale-independent EXTRACT(DOW) comparison.
-- ============================================================
