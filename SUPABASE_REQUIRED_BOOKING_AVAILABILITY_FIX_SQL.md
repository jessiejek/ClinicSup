# SUPABASE_REQUIRED_BOOKING_AVAILABILITY_FIX_SQL.md

## Purpose
Fix missing table grants and timezone-unsafe day-of-week comparisons that break patient booking date selection and walk-in slots.

## Run Order
Run ALL sections below in the Supabase SQL Editor.

---

## Section A — Grant SELECT on schedule/availability tables

The `public_doctors_view` and `doctor_available_services_view` have explicit GRANTs, but `doctor_schedules`, `doctor_blocked_dates`, and `doctor_day_statuses` do not. This causes `PublicService.getDoctorSchedules()` to fail silently (no GRANT → permission denied → `catchError` returns `[]` → all calendar dates appear disabled). The RLS policy `doctor_schedules_select_public` exists but is useless without GRANT.
...
WHERE table_name IN ('doctor_schedules','doctor_blocked_dates','doctor_day_statuses')
  AND grantee IN ('anon','authenticated')
ORDER BY table_name, grantee;
```
Expect 6 rows (3 tables × 2 roles, each with SELECT).

---

## Section B — Fix `get_available_slots` day-of-week comparison

The current implementation uses `trim(to_char(p_appointment_date, 'Day'))` which depends on the server's `lc_time` locale. If the locale is non-English, day names could differ from the `appointment_day` ENUM values. Replace with `EXTRACT(DOW FROM ...)` which returns a number (0=Sunday, 1=Monday, ..., 6=Saturday) — locale-independent.
...
## Section C — Fix `create_booking` day-of-week comparison

Same `to_char` → `EXTRACT(DOW)` fix in the `create_booking` RPC.
...
BEGIN
    ...
    LOOP
        ...
        SELECT v_booking_id, s.id, s.name, 1, s.price
...
FROM pg_proc
WHERE proname IN ('get_available_slots','create_booking')
  AND prosrc NOT LIKE '%to_char%Day%';
```
Expect both functions to NOT contain `to_char.*Day`.
...
```sql
-- Restore original grant state (if you need to revert)
REVOKE SELECT ON public.doctor_schedules FROM anon, authenticated;
REVOKE SELECT ON public.doctor_blocked_dates FROM anon, authenticated;
REVOKE SELECT ON public.doctor_day_statuses FROM anon, authenticated;
```

Note: Reverting the RPCs requires re-deploying the original SQL from `phase-02-booking-workflow.sql`.
